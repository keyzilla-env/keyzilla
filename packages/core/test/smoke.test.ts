...
describe("shared can be accessed on both server and client", () => {
  ...
  /**
   * Creates an environment object with the following properties:
   *
   * - `NODE_ENV`: The current value of `process.env.NODE_ENV`.
   * - `BAR`: The value of `process.env.BAR`.
   * - `FOO_BAR`: The value of `process.env.FOO_BAR`.
   *
   * @returns {Readonly<{NODE_ENV: "development" | "production" | "test", BAR: string, FOO_BAR: string}>}
   */
