    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                ...
                >
                    <Avatar className="h-6 w-6 mr-2">
                        <AvatarImage src={organization ? organization.imageUrl : user.imageUrl} alt={organization?.name || user.fullName || ''} />
                        <AvatarFallback>{organization?.name?.[0] || user.fullName?.[0] || 'U'}</AvatarFallback>
                    </Avatar>
            ...
            <PopoverContent className="w-[200px] p-0 ">
                ...
                <Link href="/dashboard" onClick={() => setOpen(false)} className="flex items-center justify-center w-full p-2 text-sm text-muted-foreground">
                    Return to Dashboard
                </Link>
